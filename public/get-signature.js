...
signatureField.on("mousedown", (e) => {
  paintmode = true;
  console.log("starting to paint");
  ctx.beginPath();
  ctx.moveTo(
    e.pageX - signatureField.offset().left,
    e.pageY - signatureField.offset().top
  );
});
signatureField.on("mousemove", (e) => {
  if (paintmode == true) {
...
signatureField.on("mouseleave", (e) => {
  ctx.beginPath();
});
signatureField.on("mouseenter", (e) => {
  ctx.moveTo(
    e.pageX - signatureField.offset().left,
    e.pageY - signatureField.offset().top
  );
});
wholeDocument.on("mouseup", (e) => {
  if (paintmode == true) {
    paintmode = false;
    ctx.stroke();
